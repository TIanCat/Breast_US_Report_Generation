...
import numpy as np
from transformer.Layers import EncoderLayer, DecoderLayer, EncoderLayer2
...
class CNNTransformer(nn.Module):
    ...
    def __init__(
            self, n_trg_vocab, trg_pad_idx, d_word_vec=512, d_model=512, d_inner=2048, 
            de_n_layers=6, n_head=8, d_k=64, d_v=64, dropout=0.1, 
            trg_n_position = 300, track_bn = True, use_local = False, 
    ...
    def forward(self, src_img, img_idx_g, img_seq_g, img_idx_l, img_seq_l, trg_seq):

        # visual token mask
        img_mask_g = get_pad_mask(img_seq_g, self.src_pad_idx)  # b * 1 * g
        img_mask_l = get_pad_mask(img_seq_l, self.src_pad_idx)  # b * 1 * l
        ...
        # decode
        if self.use_local:
            dec_output, *_ = self.decoder(trg_seq, trg_mask, img_feat_l, img_mask_l)
        else:
            dec_output, *_ = self.decoder(trg_seq, trg_mask, img_feat_g, img_mask_g)
