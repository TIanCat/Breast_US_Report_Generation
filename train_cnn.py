...
parser.add_argument('-map_dir', default="data/text_map") 

parser.add_argument('-target',type=str, default= "findings") 
...
def prepare_dataloaders(opt):
    ...
    if opt.target == "findings":
        stoi_path = os.path.join(opt.map_dir, 'findings_stoi_min10.json')
        opt.max_token_seq_len = opt.src_max_len
    elif opt.target == "impression":
        stoi_path = os.path.join(opt.map_dir, 'impression_stoi_min10.json')
        opt.max_token_seq_len = opt.trg_max_len
    else:
        raise

    stoi_dict = json.load(open(stoi_path, 'r',encoding='utf8'))
    opt.trg_vocab_size = len(stoi_dict)
    opt.src_pad_idx = opt.trg_pad_idx = stoi_dict[Constants.PAD_WORD]
    opt.trg_bos_idx = stoi_dict[Constants.BOS_WORD]
    opt.trg_eos_idx = stoi_dict[Constants.EOS_WORD]
    ...
    train_iterator = DataLoader(train,batch_size=opt.batch_size, shuffle=True,num_workers=16,
...
def prepare_model(opt):
    model = CNNTransformer(
        ...
        d_v=opt.d_v,
        dropout=opt.dropout,
        trg_n_position = opt.max_token_seq_len,
...
def train_epoch(model, training_data, optimizer, opt, smoothing):
    ...
    for batch in tqdm(training_data, mininterval=2, desc=desc, leave=False):
        ...
        target_seq, gold = target_trans(src_seq, trg_seq, opt)

        optimizer.zero_grad()
        # forward
        pred = model(src_img, img_idx_g, img_seq_g, img_idx_l, img_seq_l, target_seq)
        # cal loss & acc
    ...
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=2, desc=desc, leave=False):
            ...
            for i in range(len(batch)):
                ...
                for idxx,item in enumerate(temp_seq):
                    if item == 3:
                        temp_seq = temp_seq[1:idxx]
                        break
                    
                target_seqs.append(temp_seq)
...
def target_trans(src_seq, trg_seq, opt):
    if opt.target == "findings":
        target_seq = src_seq
    elif opt.target == "impression":
        target_seq = trg_seq
        ...
        raise
    target_seq, gold = patch_trg(target_seq, opt.trg_pad_idx)
    return target_seq, gold
